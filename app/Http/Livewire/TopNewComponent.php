...
class TopNewComponent extends Component
{
    use WithPagination;
    ...
    {
        $dateline =date('Y-m-d H:i:s',strtotime('-5 day',strtotime(date('Y-m-d H:i:s'))));
        if($this->sort=="default"){
            $products = Products::with('product_images')->inRandomOrder()
            ->where('created_at','>',$dateline)
            ->where('quantity','>',0)
            ->where('name','like','%'.$this->search.'%')->paginate(20);
        }
        else if($this->sort=="popularity"){
            $products = Products::with('product_images')->where('created_at','>',$dateline)
            ->where('name','like','%'.$this->search.'%')
            ->where('quantity','>',0)
            ->orderBy('view','ASC')->paginate(20);
